 ...
 */
public class Mapbox_BasicMercatorTiledImageLayer extends Mapbox_MercatorTiledImageLayer
{
    private final Object fileLock = new Object();

    public Mapbox_BasicMercatorTiledImageLayer(LevelSet levelSet)
    {
        super(levelSet);

        if (!WorldWind.getMemoryCacheSet().containsCache(Mapbox_MercatorTextureTile.class.getName()))
        {
            long size = Configuration.getLongValue(
                AVKey.TEXTURE_IMAGE_CACHE_SIZE, 3000000L);
            MemoryCache cache = new BasicMemoryCache((long) (0.85 * size), size);
            cache.setName("Texture Tiles");
            WorldWind.getMemoryCacheSet().addCache(Mapbox_MercatorTextureTile.class.getName(), cache);
        }
    }

    public Mapbox_BasicMercatorTiledImageLayer(AVList params)
    {
        this(new LevelSet(params));
        this.setValue(AVKey.CONSTRUCTION_PARAMETERS, params);
    }

    protected void forceTextureLoad(Mapbox_MercatorTextureTile tile)
    {
    ...
    protected void requestTexture(DrawContext dc, Mapbox_MercatorTextureTile tile)
    {
    ...
    {
        private final Mapbox_BasicMercatorTiledImageLayer layer;
        private final Mapbox_MercatorTextureTile tile;

        private RequestTask(Mapbox_MercatorTextureTile tile,
            Mapbox_BasicMercatorTiledImageLayer layer)
        {
            this.layer = layer;
            this.tile = tile;
        }
    ...
    private boolean isTextureExpired(Mapbox_MercatorTextureTile tile,
    ...
    private boolean loadTexture(Mapbox_MercatorTextureTile tile,
        java.net.URL textureURL)
    {
    ...
    private void addTileToCache(Mapbox_MercatorTextureTile tile)
    {
        WorldWind.getMemoryCache(Mapbox_MercatorTextureTile.class.getName()).add(
            tile.getTileKey(), tile);
    }

    protected void downloadTexture(final Mapbox_MercatorTextureTile tile)
    {
...
//        if ("http".equalsIgnoreCase(url.getProtocol()))
        ...
        RetrievalPostProcessor
    {
        ...
        public DownloadPostProcessor(Mapbox_MercatorTextureTile tile,
            Mapbox_BasicMercatorTiledImageLayer layer)
        {
            this.tile = tile;
            this.layer = layer;
