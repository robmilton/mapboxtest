 ...
 * Copied by BridgeNet March 12, 2015
 */
public abstract class Mapbox_MercatorTiledImageLayer extends AbstractLayer
{
    // Infrastructure
    private static final LevelComparer levelComparer = new LevelComparer();
    private final LevelSet levels;
    private ArrayList<Mapbox_MercatorTextureTile> topLevels;
    ...
    private ArrayList<Mapbox_MercatorTextureTile> currentTiles = new ArrayList<Mapbox_MercatorTextureTile>();
    private Mapbox_MercatorTextureTile currentResourceTile;
    ...
    abstract protected void requestTexture(DrawContext dc, Mapbox_MercatorTextureTile tile);

    abstract protected void forceTextureLoad(Mapbox_MercatorTextureTile tile);

    public Mapbox_MercatorTiledImageLayer(LevelSet levelSet)
    {
        if (levelSet == null)
        {
    ...
    private void createTopLevelTiles()
    {
        ...
        this.topLevels = new ArrayList<Mapbox_MercatorTextureTile>(nLatTiles
            * nLonTiles);
        ...
        for (int row = firstRow; row <= lastRow; row++)
        {
            ...
            for (int col = firstCol; col <= lastCol; col++)
            {
                ...
                this.topLevels.add(new Mapbox_MercatorTextureTile(new MercatorSector(
    ...
    {
        for (Mapbox_MercatorTextureTile tile : this.topLevels)
        {
    ...
    private void addTileOrDescendants(DrawContext dc, Mapbox_MercatorTextureTile tile)
    {
        ...
        Mapbox_MercatorTextureTile ancestorResource = null;
        ...
        try
        {
            ...
            Mapbox_MercatorTextureTile[] subTiles = tile.createSubTiles(this.levels.getLevel(tile.getLevelNumber() + 1));
            for (Mapbox_MercatorTextureTile child : subTiles)
            {
    ...
    private void addTile(DrawContext dc, Mapbox_MercatorTextureTile tile)
    {
    ...
    private void addTileToCurrent(Mapbox_MercatorTextureTile tile)
    {
        this.currentTiles.add(tile);
    }

    private boolean isTileVisible(DrawContext dc, Mapbox_MercatorTextureTile tile)
    {
    ...
    private boolean meetsRenderCriteria(DrawContext dc, Mapbox_MercatorTextureTile tile)
    {
    ...
    private void draw(DrawContext dc)
    {
        ...
        {
            Mapbox_MercatorTextureTile[] sortedTiles = new Mapbox_MercatorTextureTile[this.currentTiles
                .size()];
    ...
    private static class LevelComparer implements
        Comparator<Mapbox_MercatorTextureTile>
    {
        public int compare(Mapbox_MercatorTextureTile ta, Mapbox_MercatorTextureTile tb)
        {
    ...
    private void drawTileIDs(DrawContext dc,
        ArrayList<Mapbox_MercatorTextureTile> tiles)
    {
        java.awt.Rectangle viewport = dc.getView().getViewport();
        TextRenderer textRenderer = OGLTextRenderer.getOrCreateTextRenderer(dc.getTextRendererCache(), java.awt.Font.decode("Arial-Plain-13"));
        ...
        for (Mapbox_MercatorTextureTile tile : tiles)
        {
        ...
        ArrayList<Mapbox_MercatorTextureTile> tiles)
    {
        ...
        gl.glGetFloatv(gl.GL_CURRENT_COLOR, previousColor, 0);
        gl.glColor3d(0, 1, 0);
    ...
    private BufferedImage requestImage(Mapbox_MercatorTextureTile tile, String mimeType)
    ...
    private void downloadImage(final Mapbox_MercatorTextureTile tile, String mimeType)
        throws Exception
    {
        ...
        boolean abortOnError, BufferedImage image)
    {
        ...
        Mapbox_MercatorTextureTile[][] tiles = this.getTilesInSector(sector,
            levelNumber);
        ...
        for (Mapbox_MercatorTextureTile[] row : tiles)
        {
            for (Mapbox_MercatorTextureTile tile : row)
            {
                if (tile == null)
                    continue;
    ...
    private Mapbox_MercatorTextureTile[][] getTilesInSector(Sector sector,
        int levelNumber)
    {
        if (sector == null)
        {
        ...
        int numRows = nwRow - seRow + 1;
        int numCols = seCol - nwCol + 1;
        Mapbox_MercatorTextureTile[][] sectorTiles = new Mapbox_MercatorTextureTile[numRows][numCols];
        ...
        for (int row = nwRow; row >= seRow; row--)
        {
            for (int col = nwCol; col <= seCol; col++)
            {
                ...
                sectorTiles[nwRow - row][col - nwCol] = new Mapbox_MercatorTextureTile(
    ...
    private BufferedImage getImage(Mapbox_MercatorTextureTile tile, String mimeType)
        throws Exception
    {
    ...
    {
        private Mapbox_MercatorTextureTile tile;

        public HttpRetrievalPostProcessor(Mapbox_MercatorTextureTile tile)
        {
            this.tile = tile;
        }
        ...
        public ByteBuffer run(Retriever retriever)
        {
            ...
            if (htr.getResponseCode() == HttpURLConnection.HTTP_NO_CONTENT)
            {
                ...
                Mapbox_MercatorTiledImageLayer.this.levels.markResourceAbsent(tile);
                return null;
            }
